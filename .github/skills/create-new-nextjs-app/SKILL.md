...
この際、`.next` や `node_modules`、`.gitignore` などの不要なファイルは削除するようにしています。
`.next`, `node_modules` はビルドの成果物や依存関係のフォルダであり、移動させる必要はないため、削除して問題ありません。
また、`.gitignore` は、移動先にすでに存在する場合が多いため、ここでは削除する例としていますが、
状況によっては、差分を確認して、必要な部分だけをマージする方法を取ることもできます。
